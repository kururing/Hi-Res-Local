...
export class IncrementalSha256 {
  ...
  digest(): Uint8Array {
    const bitLen = BigInt(this.bytesHashed) * 8n;
    this.update(new Uint8Array([0x80]));
    const zeros = this.bufferLength > 56 ? 64 - this.bufferLength + 56 : 56 - this.bufferLength;
    if (zeros) this.update(new Uint8Array(zeros));
    const len = new Uint8Array(8);
    new DataView(len.buffer).setBigUint64(0, bitLen);
    this.update(len);
    const out = new Uint8Array(32);
    const view = new DataView(out.buffer);
    for (let i = 0; i < 8; i += 1) view.setUint32(i * 4, this.state[i] ?? 0);
    return out;
  }
