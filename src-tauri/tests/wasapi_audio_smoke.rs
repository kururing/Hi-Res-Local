//! Smoke tests for nnpm-audio-core decode + WASAPI exclusive negotiation (Windows).
...
use nghenhacpromax_lib::audio::pcm_convert::{synthetic_stereo_pcm, ChannelIdentityPattern};
...
#[test]
fn core_pcm24_packs_high_valid_bits_without_static() {
    let dir = tempfile::tempdir().unwrap();
    ...
    decoder
        .prime_bit_perfect()
        .expect("verify decoded 24-bit representation");
    decoder
...
fn synthetic_cases() -> Vec<(AudioFormat, bool)> {
    vec![
        (AudioFormat::s16(44_100, 2), false),
        (AudioFormat::s16(48_000, 2), false),
        (AudioFormat::s24_in_32(44_100, 2), false),
        (AudioFormat::s24_in_32(48_000, 2), false),
        (AudioFormat::s32(44_100, 2), false),
        (AudioFormat::s32(48_000, 2), false),
    ]
}

fn assert_synthetic_lr(pattern: ChannelIdentityPattern, format: &AudioFormat, packed: bool) {
    let mut bytes = Vec::new();
    synthetic_stereo_pcm(pattern, format, packed, 32, &mut bytes);
    let bpf = format.bytes_per_frame_packed(packed);
    assert_eq!(bytes.len() % bpf, 0);
    assert!(bpf >= 4, "stereo frame must hold L and R");
    let bps = if packed { 3 } else { format.bytes_per_sample() };
    match pattern {
        ChannelIdentityPattern::LeftSineRightSilence => {
            let mut any_left = false;
            for frame in bytes.chunks_exact(bpf) {
                let right_zero = frame[bps..bpf].iter().all(|b| *b == 0);
                assert!(right_zero, "right must be silence");
                any_left |= frame[..bps].iter().any(|b| *b != 0);
            }
            assert!(any_left, "left must carry the sine");
        }
        ChannelIdentityPattern::LeftSilenceRightSine => {
            let mut any_right = false;
            for frame in bytes.chunks_exact(bpf) {
                let left_zero = frame[..bps].iter().all(|b| *b == 0);
                assert!(left_zero, "left must be silence");
                any_right |= frame[bps..bpf].iter().any(|b| *b != 0);
            }
            assert!(any_right, "right must carry the sine");
        }
        ChannelIdentityPattern::LeftPosRightNeg => {
            for frame in bytes.chunks_exact(bpf) {
                assert_ne!(&frame[..bps], &frame[bps..bpf]);
            }
        }
    }
}
...
#[test]
fn synthetic_channel_patterns_match_pcm_wire_without_decoder() {
    for (fmt, packed) in synthetic_cases() {
        assert_synthetic_lr(ChannelIdentityPattern::LeftSineRightSilence, &fmt, packed);
        assert_synthetic_lr(ChannelIdentityPattern::LeftSilenceRightSine, &fmt, packed);
        assert_synthetic_lr(ChannelIdentityPattern::LeftPosRightNeg, &fmt, packed);
    }
}

#[test]
fn synthetic_channel_identity_exclusive_injects_without_decoder() {
    let _guard = audio_device_guard();
    let mgr = WasapiDeviceManager::new();
    let device = match mgr.get_active_device() {
        Ok(d) => d,
    ...
    for (src, _) in synthetic_cases() {
        let negotiated = match FormatNegotiator::negotiate(&device, &src, false) {
            Ok(n) => n,
            Err(err) if skip_busy_endpoint(&err) => return,
            Err(_) => continue,
        };
        let packed = negotiated.packed_s24;
        let mut pcm = Vec::new();
        ...
        let bpf = negotiated.bytes_per_frame();
        assert_eq!(pcm.len() % bpf, 0);
        assert_eq!(
            negotiated
                .wave
        ...
        let ring = PcmRing::for_wire(negotiated.format.sample_rate, bpf);
        let (mut producer, consumer) = ring.split();
        let written = producer.push_bytes_aligned(&pcm, bpf);
        assert_eq!(written % bpf, 0);
