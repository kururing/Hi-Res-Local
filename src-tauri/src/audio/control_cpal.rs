...
enum ControlCommand {
    ...
    SetEnabled(bool, Sender<AudioResult<()>>),
...
impl AudioControlHandle {
    ...
    pub fn set_enabled(&self, enabled: bool) -> AudioResult<()> {
        self.request(|reply| ControlCommand::SetEnabled(enabled, reply))
    }
    ...
    pub fn set_paused(&self, _paused: bool) {
        // CPAL shared path uses pipeline.is_playing for pause silence.
    }
    ...
    pub fn set_exclusive_mode(
        &self,
        _enabled: bool,
        _source: crate::audio::pcm::AudioFormat,
    ) -> AudioResult<Option<()>> {
    ...
    while let Ok(command) = rx.recv() {
        match command {
            ControlCommand::EnsureStream(reply) => {
                let result = if !enabled || active_stream.is_some() {
                    Ok(())
                } else {
            ...
            ControlCommand::SetEnabled(next_enabled, reply) => {
            ...
            ControlCommand::IsActive(reply) => {
                let _ = reply.send(enabled && active_stream.is_some());
            }
